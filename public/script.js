// Konfigurasi Google Sheets - Ganti dengan URL Web App Apps Script Anda
...
function getKategoriMarkerIcon(kategori, status) {
    ...
    return L.divIcon({
        ...
        html: `<div style="position:relative;width:44px;height:44px;display:flex;align-items:center;justify-content:center;cursor:pointer;">
            <div style="position:absolute;inset:-2px;border-radius:50%;background:${statusColor}33;border:2px solid ${statusColor};box-shadow:0 0 0 2px #fff,0 0 0 4px ${statusColor};transition:transform 0.2s ease;"></div>
            <div style="position:absolute;inset:0;border-radius:50%;background:${meta.bg};border:2px solid ${meta.color};display:flex;align-items:center;justify-content:center;">
...
function validateLocation(lat, lng) {
    ...
    for (const polygon of WILAYAH.ngambon.polygons) {
        if (isPointInPolygon(lat, lng, polygon)) return { valid: true, kecamatan: 'Kecamatan Ngambon' };
    }
    for (const polygon of WILAYAH.tambakrejo.polygons) {
        if (isPointInPolygon(lat, lng, polygon)) return { valid: true, kecamatan: 'Kecamatan Tambakrejo' };
    }
    return { valid: false, kecamatan: null };
}
...
function startLiveTracking() {
    ...
    liveWatchId = navigator.geolocation.watchPosition((pos) => {
        ...
        if (liveCircle) { liveCircle.setLatLng([lat, lng]); liveCircle.setRadius(accuracy); } else { 
            liveCircle = L.circle([lat, lng], { radius: accuracy, color: '#3b82f6', fillColor: '#3b82f6', fillOpacity: 0.08, weight: 2, dashArray: '6 4' }).addTo(map); 
        }
        if (!liveMarkerEl) {
        ...
        if (error.code === error.PERMISSION_DENIED) pesan += 'Izin lokasi ditolak.'; 
        else if (error.code === error.POSITION_UNAVAILABLE) pesan += 'Sinyal GPS tidak tersedia.'; 
        else pesan += 'Waktu habis.'; 
...
function renderLaporan(laporan) {
    ...
    laporanDiv.innerHTML = laporan.map((l, index) => { 
        ...
        const kategoriData = KATEGORI_ICON_MAP[l.kategori] || KATEGORI_ICON_MAP['lainnya'];
        return `<div class="laporan-card" data-aos="fade-up" data-aos-delay="${Math.min(index * 50, 500)}">
            <div class="laporan-header">
                <div class="laporan-user"><i class="fas fa-user-circle"></i><strong>${escapeHtml(l.nama || 'Anonim')}</strong></div>
                <span class="laporan-status status-${statusClass}">${l.status === 'belum diproses' ? '⏳ Menunggu Verifikasi' : (l.status === 'proses' ? '🔄 Sedang Diproses DLH' : '✅ Selesai')}</span>
            </div>
            <div class="laporan-body">
                <div class="laporan-lokasi"><i class="fas fa-map-marker-alt"></i><span>📍 ${escapeHtml(l.lokasi || 'Lokasi tidak tersedia')}</span></div>
                <div>
                    ${l.kecamatan ? `<span class="laporan-badge"><i class="fas fa-building"></i> ${escapeHtml(l.kecamatan)}</span>` : ''}
                    ${l.kategori ? `<span class="laporan-badge">${kategoriData.emoji} ${kategoriData.label}</span>` : ''}
                ...
                ${l.foto && l.foto.startsWith('http') ? `<div class="laporan-foto" onclick="window.openImageModal('${l.foto.replace(/'/g, "\\'")}')"><img src="${l.foto}" alt="Dokumentasi sampah" loading="lazy" onerror="this.style.display='none'"></div>` : ''}
            </div>
            <div class="laporan-footer">
                <span><i class="far fa-calendar-alt"></i> ${l.timestamp ? new Date(l.timestamp).toLocaleDateString('id-ID') : 'Tanggal tidak tersedia'}</span>
...
function filterLaporan() {
    ...
    const filtered = allLaporan.filter(l => { 
        const matchSearch = (l.deskripsi || '').toLowerCase().includes(searchTerm) || (l.lokasi || '').toLowerCase().includes(searchTerm) || (l.nama || '').toLowerCase().includes(searchTerm); 
...
function updateMapMarkers(laporan) {
    if (!map) {
        console.log('Map belum siap');
        return;
    }
    
    console.log('Update map markers dengan', laporan.length, 'laporan');
    ...
    laporan.forEach(l => {
        ...
        const popupContent = `<div style="min-width:260px;max-width:320px;font-family:'Inter',sans-serif;padding:4px;">
            <div style="display:flex;align-items:center;gap:10px;margin-bottom:12px;border-bottom:2px solid #e2e8f0;padding-bottom:8px;">
                <div style="width:40px;height:40px;background:${meta.bg};border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:22px;">${meta.emoji}</div>
                <div><div style="font-weight:800;font-size:14px;color:#1e293b;">${meta.label}</div><div style="font-size:11px;color:#64748b;">${statusText}</div></div>
            </div>
            <div style="margin-bottom:8px;display:flex;align-items:center;gap:8px;"><i class="fas fa-user" style="color:#10b981;width:16px;"></i><span style="font-weight:600;font-size:13px;">${escapeHtml(l.nama || 'Anonim')}</span></div>
            ${l.kecamatan ? `<div style="margin-bottom:8px;display:flex;align-items:center;gap:8px;"><i class="fas fa-building" style="color:#10b981;width:16px;"></i><span style="font-size:12px;color:#475569;">${escapeHtml(l.kecamatan)}</span></div>` : ''}
            <div style="margin-bottom:8px;display:flex;align-items:flex-start;gap:8px;"><i class="fas fa-align-left" style="color:#10b981;width:16px;margin-top:2px;"></i><span style="font-size:12px;color:#475569;line-height:1.5;">${escapeHtml((l.deskripsi || '').substring(0, 100))}${(l.deskripsi || '').length > 100 ? '…' : ''}</span></div>
            ${l.volume && l.volume !== 'tidak diketahui' ? `<div style="margin-bottom:8px;display:flex;align-items:center;gap:8px;"><i class="fas fa-truck" style="color:#10b981;width:16px;"></i><span style="font-size:11px;background:#f1f5f9;padding:2px 8px;border-radius:20px;">${getVolumeText(l.volume)}</span></div>` : ''}
            <hr style="margin:10px 0;border-color:#e2e8f0;">
            <div style="display:flex;justify-content:space-between;align-items:center;font-size:10px;color:#94a3b8;">
                <span><i class="far fa-calendar-alt"></i> ${l.timestamp ? new Date(l.timestamp).toLocaleDateString('id-ID', { day: '2-digit', month: 'short', year: 'numeric' }) : '—'}</span>
                <a href="#daftar" style="color:#10b981;text-decoration:none;font-weight:600;"><i class="fas fa-eye"></i> Detail</a>
            </div>
        </div>`;
        
        const markerIcon = getKategoriMarkerIcon(l.kategori, l.status);
...
function setupEventListeners() {
    ...
    if (searchInput && searchClear) { 
        searchInput.addEventListener('input', () => { if (searchClear) searchClear.style.display = searchInput.value ? 'flex' : 'none'; }); 
        searchClear.addEventListener('click', () => { if (searchInput) searchInput.value = ''; filterLaporan(); if (searchClear) searchClear.style.display = 'none'; }); 
    }
    ...
    window.addEventListener('scroll', () => { 
        ...
        sections.forEach(section => { 
            ...
            if (scrollPos >= sectionTop && scrollPos < sectionBottom && sectionId) { 
                document.querySelectorAll('.nav-link').forEach(link => { link.classList.remove('active'); if (link.getAttribute('href') === `#${sectionId}`) link.classList.add('active'); }); 
            } 
        }); 
    });
...
function setupScrollEffect() {
    window.addEventListener('scroll', () => { 
        if (window.scrollY > 300) { if (scrollTopBtn) scrollTopBtn.classList.add('show'); } else { if (scrollTopBtn) scrollTopBtn.classList.remove('show'); } 
        if (window.scrollY > 50) { if (navbar) navbar.classList.add('scrolled'); } else { if (navbar) navbar.classList.remove('scrolled'); } 
    });
}
