...
function checkLogin() {
    ...
    if (ok === 'true' && time) {
        const elapsed = (Date.now() - parseInt(time, 10)) / 3600000;
...
function handleNavClick(e) {
    ...
    if (headerBreadcrumb) headerBreadcrumb.innerHTML = `<i class="fas ${icons[page] || 'fa-circle'}"></i> ${titles[page] || page}`;
...
async function loadData(silent = false) {
    ...
    if (!silent && tbody) {
        tbody.innerHTML = `<tr><td colspan="9" class="loading-cell"><span class="loading-spinner"><i class="fas fa-spinner fa-spin"></i></span><span>Memuat data...</span></td></tr>`;
    }
    ...
    try {
        ...
        if (!silent && tbody) {
            tbody.innerHTML = `<tr><td colspan="9" class="loading-cell"><span class="loading-spinner"><i class="fas fa-triangle-exclamation"></i></span><span>Gagal memuat data. Periksa koneksi.</span></td></tr>`;
        }
        ...
        options: {
            ...
            plugins: {
                legend: {
                    ...
                    position: 'bottom',
                    labels: { padding: 18, font: { size: 12 }, boxWidth: 12, boxHeight: 12, borderRadius: 4, usePointStyle: true }
                },
                tooltip: {
                    backgroundColor: '#0f2117',
                    padding: 11,
                    cornerRadius: 10,
                    callbacks: { label: ctx => ` ${type === 'doughnut' ? ctx.parsed : ctx.parsed.y} laporan` }
                }
            },
            scales: type !== 'doughnut' ? {
                y: { beginAtZero: true, ticks: { stepSize: 1, font: { size: 11 } }, grid: { color: 'rgba(0,0,0,.04)' }, border: { display: false } },
...
function updateStatistikCharts() {
    ...
    renderChart('statStatusChart', 'doughnut', ['Menunggu', 'Diproses', 'Selesai'], [
    ...
    if (selesaiLaporan.length) {
        const totalHours = selesaiLaporan.reduce((acc, l) => acc + (Date.now() - new Date(l.timestamp).getTime()) / 3600000, 0);
...
function renderTable() {
    ...
    filteredCache = allLaporan.filter(l => {
        const matchSearch = (l.deskripsi || '').toLowerCase().includes(search) ||
    ...
    if (filteredCache.length === 0) {
        tbody.innerHTML = `<tr><td colspan="9" class="loading-cell"><span class="loading-spinner"><i class="fas fa-inbox"></i></span><span>Tidak ada laporan</span></td></tr>`;
        return;
    }
    ...
    tbody.innerHTML = filteredCache.map((l, i) => {
        return `
        <tr>
            <td><span style="font-family:monospace;font-size:12px;">${String(i + 1).padStart(2, '0')}</span></td>
            <td style="white-space:nowrap;font-size:12px;">${l.timestamp ? new Date(l.timestamp).toLocaleDateString('id-ID') + '<br><span style="color:#8ca89a">' + new Date(l.timestamp).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' }) + '</span>' : '—'}</td>
            <td>${escapeHtml(l.kecamatan || '—')}</td>
            <td style="max-width:140px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="${escapeAttr(l.lokasi)}">${escapeHtml((l.lokasi || '').substring(0, 30))}${(l.lokasi || '').length > 30 ? '…' : ''}</td>
            <td style="max-width:180px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="${escapeAttr(l.deskripsi)}">${escapeHtml((l.deskripsi || '').substring(0, 45))}${(l.deskripsi || '').length > 45 ? '…' : ''}</td>
            <td>${getKategoriName(l.kategori)}</td>
            <td>${escapeHtml(l.nama || 'Anonim')}</td>
            <td>
    ...
    overlay.innerHTML = `
        <div class="modal-box">
            <div class="modal-header">
                <h3><i class="fas fa-file-lines"></i> Detail Laporan</h3>
            ...
            ${l.foto ? `<div class="modal-row"><div class="modal-row-label"><i class="fas fa-image"></i>Foto</div><div class="modal-row-value"><img src="${getDriveImgUrl(l.foto)}" alt="Dokumentasi" class="modal-photo" onerror="this.parentElement.innerHTML='<a href=\\'${escapeAttr(l.foto)}\\' target=\\'_blank\\'>Buka di Drive →</a>'"></div></div>` : ''}
            <button class="modal-btn-close" onclick="this.closest('.modal-overlay').remove()"><i class="fas fa-xmark"></i> Tutup</button>
        </div>
    `;
...
function row(icon, label, value) {
    return `<div class="modal-row"><div class="modal-row-label"><i class="fas ${icon}"></i>${label}</div><div class="modal-row-value">${escapeHtml(String(value))}</div></div>`;
}
...
function confirmDelete(filteredIdx) {
    ...
    overlay.innerHTML = `
        <div class="modal-box" style="max-width:400px;">
            ...
            <p class="modal-delete-desc">Laporan dari <strong>${escapeHtml(l.nama || 'Anonim')}</strong> akan dihapus permanen.</p>
            <div class="modal-btn-group">
                <button class="modal-btn-cancel" onclick="this.closest('.modal-overlay').remove()"><i class="fas fa-xmark"></i> Batal</button>
                <button class="modal-btn-confirm" id="_confirmHapus"><i class="fas fa-trash-can"></i> Ya, Hapus</button>
            </div>
        </div>
    `;
    document.body.appendChild(overlay);
...
async function deleteReport(timestamp) {
    ...
    try {
        ...
        const response = await fetch(`${SCRIPT_URL}?${params}`);
        const result = await response.json();
        if (!result.success) throw new Error(result.error || 'Gagal hapus');
...
function exportToCSV() {
    if (allLaporan.length === 0) { showToast('Tidak ada data', 'error'); return; }
    ...
    showToast(`Ekspor ${allLaporan.length} data berhasil`, 'success');
}
...
function getStatusText(s) {
    const map = { 'belum diproses': 'Menunggu', 'proses': 'Diproses', 'selesai': 'Selesai' };
    return map[s] || s;
}
...
function showToast(msg, type = 'success') {
    ...
    toast.innerHTML = `<i class="fas ${type === 'success' ? 'fa-circle-check' : 'fa-circle-exclamation'}"></i> ${escapeHtml(msg)}`;
